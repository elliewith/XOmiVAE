...
class DeepExplainer(Explainer):
    ...
    def __init__(self, model, data,outputNumber, dim=None ,explainLatentSpace=False, session=None, learning_phase_flags=None):
        ...
        model : if framework == 'tensorflow', (input : [tf.Tensor], output : tf.Tensor)
            ...
            If the input is a tuple, the returned shapLundberg values will be for the input of the
        ...
        if framework == 'tensorflow':
            ...
            self.explainer = PyTorchDeepExplainer(model, data,outputNumber,dim,explainLatentSpace)
